...
# Code sandwiched between lines of #####, !!!!!! and ***** can be uncommented to make adjustments to resampling, shuffling, and
...
def runNNmodel(featureTrain, targetTrain, featureTest, targetTest, predictFeatures, predictTargets, 
    ...
    plt.show()
    fig.savefig(r'C:\Users\Samantha.Longridge\Documents\Python_Scripts\WLAI\GitHub_Files\results\NN_modelTrainingHistory.png')
    ...
    plt.show()
    fig.savefig(r'C:\Users\Samantha.Longridge\Documents\Python_Scripts\WLAI\GitHub_Files\results\ConfusionMatrix.png')
...
# 200000 is the total number of points you want to train the model with
# 200000 gives 95% good data points and 5% bad data points, reducing to 100000 gives 90% good and 10% bad
...
# Shuffle data                 # shuffles the data so that it's not in order
...
# If you are not shuffling the training and validation data above, uncomment the code below and comment out the set of code above

#featureNames = ['PRIMARY','PRIMARY_SIGMA','PRIMARY_SIGMA_TRUE','PRIMARY_RESIDUAL','BACKUP','BACKUP_TRUE','PREDICTION',]

#featureTrain=trainAll.loc[:, featureNames]
#featureVal=validateAll.loc[:, featureNames]

#targetTrain=trainAll.loc[:,['TARGET']]
#targetVal=validateAll.loc[:,['TARGET']]
...
# **************************************************************************************************************************** #
# Scaling using Min and Max - need to uncomment code below if you are scaling / normalizing

#from sklearn.preprocessing import MinMaxScaler

#scaler = MinMaxScaler(feature_range=(0, 1))  # setting the scaler
#scaler = scaler.fit(featureTrain)  # fit the scaler to the training set
#n_featureTrain = scaler.transform(featureTrain)
#n_featureTrain = pd.DataFrame(n_featureTrain)
#n_featureTrain.columns = ['PRIMARY','PRIMARY_SIGMA','PRIMARY_SIGMA_TRUE','PRIMARY_RESIDUAL','BACKUP','BACKUP_TRUE','PREDICTION']

#scaler = scaler.fit(featureVal) # fit the scaler to the validation set
#n_featureVal = scaler.transform(featureVal)
#n_featureVal = pd.DataFrame(n_featureVal)
#n_featureVal.columns = ['PRIMARY','PRIMARY_SIGMA','PRIMARY_SIGMA_TRUE','PRIMARY_RESIDUAL','BACKUP','BACKUP_TRUE','PREDICTION']

#scaler = scaler.fit(predictFeatures) # fit the scaler to the predict set
#n_predictFeatures = scaler.transform(predictFeatures)
#n_predictFeatures = pd.DataFrame(n_predictFeatures)
#n_predictFeatures.columns = ['PRIMARY','PRIMARY_SIGMA','PRIMARY_SIGMA_TRUE','PRIMARY_RESIDUAL','BACKUP','BACKUP_TRUE','PREDICTION']
...
# **************************************************************************************************************************** #
# if you scaled the data above (the code in between the two *** lines) uncomment the code below and comment out the runNNmodel function above

#modelOut_simple, model_simple = runNNmodel(n_featureTrain, targetTrain, n_featureVal, targetVal, n_predictFeatures, predictTargets,
#               predictStations, epochCount, batchCount, classThreshold, numFeatures)
...
modelPerformance    
modelPerformance.to_csv(r'C:\Users\Samantha.Longridge\Documents\Python_Scripts\WLAI\GitHub_Files\results\modelPerformance.csv')
